...
- Review and paginate orders, optionally filtered by status (pagination works correctly across all pages)
- Approve/reject pending receipts (rejection reason is captured via FSM and shown to the customer)
- Add new configs (validated to start with `vless://`, `vmess://`, or `trojan://`) against a chosen product, or delete unassigned ones (deletion is blocked for configs already assigned to a customer)
...
- Browse the 10 most recently registered users — **currently broken, see [Known Issues](KNOWN_ISSUES.md)**
...
app/
  ...
  services/
    user_service.py           # get_or_create_user (refreshes profile fields on every visit)
    ...
    product_service.py            # product-related helpers (currently unused by any handler)
...
KNOWN_ISSUES.md                     # currently-open bugs and dead code (see below)
```
...
- `admin_statistics.py` — shop-wide stats and recent users (the "recent users" screen currently crashes — see Known Issues)
...
- **User** — Telegram identity (`telegram_id`, username, first/last name). `is_admin` is a property that always returns `False`; actual admin status is decided purely by `ADMIN_IDS` in settings, not stored per-user. Profile fields (`username`/`first_name`/`last_name`) are refreshed on every `/start` so they don't go stale.
- **Product** — name, description, price (`Numeric(10,2)`, currently typed as `float` in Python — see Known Issues), currency (defaults to `"تومان"`), duration in days, protocol label, active flag.
- **Config** — a single credential string (`config_text`), tied to a product, with status `AVAILABLE` or `ASSIGNED`, and once assigned, the owning user's Telegram ID and the order that consumed it. Deletion is blocked unless the config is still `AVAILABLE`.
- **Order** — the purchase record: product, amount, unit price, currency, and a status enum (`PENDING_PAYMENT → RECEIPT_SUBMITTED → APPROVED/REJECTED/COMPLETED`, or `CANCELLED`), plus receipt file references (`receipt_file_id` and `receipt_file_unique_id`, both populated), the acting admin, and rejection reason/timestamps.
- **PaymentReceipt** — the uploaded receipt photo's Telegram `file_id`/`file_unique_id`, plus the message and chat it came from (`chat_id` is required and always set).
...
The interesting piece of business logic is `OrderService.approve_order_with_config_assignment`. On approval it opens a transaction, locks the order row (`with_for_update()`), locks and grabs one `AVAILABLE` config for that product with `with_for_update(skip_locked=True)`, flips both records' statuses, stamps `assigned_at`/`approved_at`, writes an `AdminAction` log entry, and returns the config text plus the buyer's Telegram ID so the caller can DM it to the correct chat. `skip_locked=True` means two admins approving orders for the same product at the same time won't hand out the same config or deadlock waiting on each other.
...
`pytest.ini` and `pyproject.toml` both configure `asyncio_mode = auto`, so async test functions in `tests/test_bot.py` run without extra decorators. The suite is entirely mock-based (`AsyncMock`/`MagicMock`) and does not exercise a real database or the full handler call chain — see [Known Issues](KNOWN_ISSUES.md) for why that matters.
...
- **FSM for multi-step admin input.** Both "reject an order" (collect a reason) and "add a config" (collect config text, remembering which product it's for) use aiogram's `FSMContext` to gate a plain-text follow-up message to the right handler, so admin free-text isn't misinterpreted by the wrong handler.
...
*Generated from a read of the repository source (`app/`, `alembic/`, `tests/`, and root config files), cross-checked against the two historical bug-report documents and updated to reflect the current, fixed state of the code.*
