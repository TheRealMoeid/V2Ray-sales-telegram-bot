...
async def handle_buy_config(callback: CallbackQuery, session: AsyncSession):
    ...
    try:
        ...
        await callback.message.edit_text(
            ...
            reply_markup=ProductListKeyboard.get_products_keyboard(products),
        )
        await callback.answer()
        
    except Exception as e:
        logger.error(f"Error in handle_buy_config: {e}")
...
"""
        ...
        logger.error(f"Error in handle_select_product: {e}")
        await callback.answer("❌ خطایی رخ داد", show_alert=True)
...
"""
        ...
        logger.error(f"Error in handle_retry_payment: {e}")
        await callback.answer("❌ خطایی رخ داد", show_alert=True)
